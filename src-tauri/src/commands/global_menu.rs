...
pub async fn menu_get_for_window(window_id: u64) -> Result<Vec<MenuItem>, String> {
    #[cfg(target_os = "linux")]
    {
        ...
        let proxy: zbus::Proxy<'_> = zbus::proxy::Builder::new(&conn)
            .destination("com.canonical.AppMenu.Registrar").map_err(|e: zbus::Error| e.to_string())?
            .path("/com/canonical/AppMenu/Registrar").map_err(|e: zbus::Error| e.to_string())?
            .interface("com.canonical.AppMenu.Registrar").map_err(|e: zbus::Error| e.to_string())?
...
pub async fn menu_activate_item(service: String, menu_path: String, item_id: i32) -> Result<(), String> {
    #[cfg(target_os = "linux")]
    {
        ...
        let proxy: zbus::Proxy<'_> = zbus::proxy::Builder::new(&conn)
            ...
            .map_err(|e: zbus::Error| e.to_string())?
            .path(zbus::zvariant::ObjectPath::try_from(menu_path.as_str()).map_err(|e: zbus::zvariant::Error| e.to_string())?)
            .map_err(|e: zbus::Error| e.to_string())?
            .interface("com.canonical.dbusmenu").map_err(|e: zbus::Error| e.to_string())?
            .build().await.map_err(|e: zbus::Error| e.to_string())?;
        ...
        proxy.call::<_, (i32, &str, zbus::zvariant::Value<'_>, u32), ()>("Event", &(
            ...
            0u32,
        )).await.map_err(|e: zbus::Error| e.to_string())
    }
    #[cfg(not(target_os = "linux"))]
