...
pub async fn power_action(action: String) -> Result<(), String> {
    #[cfg(target_os = "linux")]
    {
        ...
        let proxy: zbus::Proxy<'_> = zbus::proxy::Builder::new(&conn)
            .destination("org.freedesktop.login1").map_err(|e: zbus::Error| e.to_string())?
            .path("/org/freedesktop/login1").map_err(|e: zbus::Error| e.to_string())?
            .interface("org.freedesktop.login1.Manager").map_err(|e: zbus::Error| e.to_string())?
            .build()
            .await
        ...
        match action.as_str() {
            ...
            "lock" => {
                ...
                let sp: zbus::Proxy<'_> = zbus::proxy::Builder::new(&conn)
                    .destination("org.freedesktop.login1").map_err(|e: zbus::Error| e.to_string())?
                    .path(zbus::zvariant::ObjectPath::try_from(session_path.as_str()).map_err(|e: zbus::zvariant::Error| e.to_string())?).map_err(|e: zbus::Error| e.to_string())?
                    .interface("org.freedesktop.login1.Session").map_err(|e: zbus::Error| e.to_string())?
                    .build()
                    .await
                    .map_err(|e: zbus::Error| e.to_string())?;
                sp.call::<_, (), ()>("Lock", &()).await.map_err(|e: zbus::Error| e.to_string())?;
            }
            "logout" => {
                let session_path = get_session_path(&conn).await?;
                let sp: zbus::Proxy<'_> = zbus::proxy::Builder::new(&conn)
                    ...
                    .await
                    .map_err(|e: zbus::Error| e.to_string())?;
                sp.call::<_, (), ()>("Terminate", &()).await.map_err(|e: zbus::Error| e.to_string())?;
            }
...
pub async fn power_get_profile() -> Result<PowerProfile, String> {
    #[cfg(target_os = "linux")]
    {
        let conn = zbus::Connection::system().await.map_err(|e: zbus::Error| e.to_string())?;
        let proxy: zbus::Proxy<'_> = zbus::proxy::Builder::new(&conn)
            .destination("net.hadess.PowerProfiles").map_err(|e: zbus::Error| e.to_string())?
            .path("/net/hadess/PowerProfiles").map_err(|e: zbus::Error| e.to_string())?
            .interface("net.hadess.PowerProfiles").map_err(|e: zbus::Error| e.to_string())?
            .build()
            .await
            .map_err(|e: zbus::Error| e.to_string())?;
